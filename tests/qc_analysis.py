...
if file_ext == '.h5ad':
    ...
    print(f"Loading 10X MTX from directory: {input_file}")
    try:
        adata = sc.read_10x_mtx(input_file, var_names='gene_symbols', cache=False)
        print(f"Loaded 10X MTX data: {adata.n_obs} cells × {adata.n_vars} genes")
        adata.var_names_make_unique()
    except Exception as e:
        print(f"\nError loading directory: {e}")
        print("Ensure directory contains matrix.mtx, genes.tsv, and barcodes.tsv")
        sys.exit(1)
else:
